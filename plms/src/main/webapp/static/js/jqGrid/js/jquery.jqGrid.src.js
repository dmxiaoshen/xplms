	...
	return this.each( function() {
		...
		setPager = function (pgid, tp){
			...
			$('input.ui-pg-input',"#"+pgcnt).keypress( function(e) {
				...
				//start add by mark
				if(!((key >= 48 && key <= 57) ||e.keyCode == 46 ||key == 8 || key== 9 ||key == 35 || key== 36)){
					return false;
				}//end add by mark
				return this;
			});
		...
		if(ts.p.autowidth===true) {
			...
			ts.p.width = pw > 0?  pw-2: 'nw';//lotus修改，减去表格两边框
		}
		setColWidth();
		$(eg).css("width",grid.width+2+"px").append("<div class='ui-jqgrid-resize-mark' id='rs_m"+ts.p.id+"'>&#160;</div>");//lotus加2
...
$.jgrid.extend({
	...
	setGridWidth : function(nwidth, shrink) {
		return this.each(function(){
			...
			$("#gbox_"+$.jgrid.jqID($t.p.id)).css("width",nwidth+2+"px");//lotus加2
...
// Date Validation Javascript
	checkDate : function (format, date) {
		...
		daysArray = function(n,_this) {
			var i;
			for (i = 1; i <= n; i++) {
				_this[i] = 31;
				if (i==4 || i==6 || i==9 || i==11) {_this[i] = 30;}
				if (i==2) {_this[i] = 29;}
			}
			return _this;
		};
